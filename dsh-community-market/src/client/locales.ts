export const zh = {
  ...
  manualInstallBody: '以下命令由 Computer Harness Desktop 根据插件信息生成，可能与仓库中的说明不同。请先检查源码，再复制到 DSH 终端执行。',
  installCommand: '安装命令',
  ...
  restartError: '无法请求重启 Computer Harness Desktop，请稍后手动重启。',
  installableBody: '这里显示从当前来源完整目录中初步筛选出的可安装插件。选择插件后，Computer Harness Desktop 仍会再次验证版本、来源和兼容性。',
  ...
  desktopRequiredTitle: '需要 Computer Harness Desktop',
  desktopUnavailable: '安装、卸载、禁用和启用只在 Computer Harness Desktop 中可用；你仍然可以浏览插件目录。',
  ...
  confirmInstallBody: '请确认 Computer Harness Desktop 验证的 npm 包、版本和目标配置。',
  confirmUninstallTitle: '确认卸载插件',
  confirmUninstallBody: '只会卸载当前配置中由插件市场安装并验证过的这个插件。',
  confirmDisableTitle: '确认禁用外部插件',
  confirmDisableBody: 'Computer Harness Desktop 将停止在当前配置中加载这个插件，但不会卸载对应的 npm 包。',
  confirmEnableTitle: '确认启用插件',
  confirmEnableBody: 'Computer Harness Desktop 将恢复在当前配置中加载这个插件。对应的代码会在重启后运行。',
  confirmInstall: '确认安装',
  ...
  restartAfterOperation: '操作完成后需要重启 Computer Harness Desktop，改动才会生效。',
  disableWarning: '禁用不会卸载 npm 包，也不会隔离插件代码。',
  disableRecoveryWarning: '如果插件导致 Computer Harness Desktop 无法进入此页面，需要按照恢复文档手动处理。',
  ...
  restartRequiredTitle: '需要重启 Computer Harness Desktop',
  restartRequiredBody: '请重启 Computer Harness Desktop，让当前配置加载最新的插件状态。',
  restartLater: '稍后重启',
  restartNow: '立即重启',
  restarting: '正在重启...',
  done: '完成',
...
export const en: Record<MarketLocaleKey, string> = {
  ...
  manualInstallBody: 'Computer Harness Desktop generated this command from the plugin information, so it may differ from the repository instructions. Review the source first, then copy it into DSH Terminal.',
  ...
  restartError: 'Computer Harness Desktop could not be restarted. Restart it manually when convenient.',
  installableBody: 'This view shows plugins pre-screened from the current source’s complete catalog. Computer Harness Desktop verifies the version, source, and compatibility again after selection.',
  ...
  desktopRequiredTitle: 'Computer Harness Desktop is required',
  desktopUnavailable: 'Install, uninstall, disable, and enable are available only in Computer Harness Desktop. You can still browse the catalog.',
  ...
  confirmInstallBody: 'Review the npm package, version, and target profile verified by Computer Harness Desktop.',
  ...
  confirmDisableBody: 'Computer Harness Desktop will stop loading this plugin in the active profile without uninstalling its npm package.',
  confirmEnableTitle: 'Confirm plugin enable',
  confirmEnableBody: 'Computer Harness Desktop will load this plugin in the active profile again. Its code will run after restart.',
  ...
  restartAfterOperation: 'Restart Computer Harness Desktop after this operation for the change to take effect.',
  disableWarning: 'Disabling does not uninstall the npm package or isolate the plugin’s code.',
  disableRecoveryWarning: 'If the plugin prevents Computer Harness Desktop from reaching this page, use the manual recovery steps.',
  ...
  restartRequiredTitle: 'Restart Computer Harness Desktop',
  restartRequiredBody: 'Restart Computer Harness Desktop so the current profile loads its updated plugin state.',
