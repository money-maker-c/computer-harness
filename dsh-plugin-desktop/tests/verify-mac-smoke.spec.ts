...
function fixture(): AppFixture {
  ...
  const contents = join(root, 'Computer Harness Desktop.app', 'Contents')
  const macos = join(contents, 'MacOS')
  const resources = join(contents, 'Resources')
  mkdirSync(macos, { recursive: true })
  mkdirSync(resources, { recursive: true })
  const infoPlist = join(contents, 'Info.plist')
  const executable = join(macos, 'Computer Harness Desktop')
...
function options(
  ...
  const value: MacSmokeVerificationOptions = {
    ...
    listDmgs: () => ['/release/dist/Computer Harness Desktop-2.0.1.dmg'],
...
describe('macOS DMG smoke artifact verification', () => {
  it('mounts one DMG and accepts a well-formed unsigned application bundle', () => {
    ...
    const appPath = join(value.root, 'Computer Harness Desktop.app')

    expect(verifyMacSmoke(harness.value)).toEqual({
      appPath,
      dmgPath: '/release/dist/Computer Harness Desktop-2.0.1.dmg',
    })
    ...
    expect(harness.calls).toEqual([
      {
        command: 'hdiutil',
        args: [
          'attach', '/release/dist/Computer Harness Desktop-2.0.1.dmg',
  ...
  it('rejects a missing Info.plist and still detaches', () => {
    ...
    expect(harness.calls).toEqual([
      {
        command: 'hdiutil',
        args: ['attach', '/release/dist/Computer Harness Desktop-2.0.1.dmg', '-mountpoint', value.root, '-nobrowse', '-readonly'],
      },
      { command: 'hdiutil', args: ['detach', value.root] },
    ])
    expect(harness.removeMountPoint).toHaveBeenCalledWith(value.root)
