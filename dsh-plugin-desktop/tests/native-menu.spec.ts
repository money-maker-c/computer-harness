...
describe('native macOS application menu', () => {
  ...
  it('localizes the complete Simplified Chinese menu while retaining native roles', () => {
    const template = macApplicationMenuTemplate('Computer Harness Desktop', 'zh-CN')

    expect(template.map(item => item.label)).toEqual([
      'Computer Harness Desktop', '文件', '编辑', '显示', '窗口',
    ])
    expect(submenu(template[0]!).map(item => item.label).filter(Boolean)).toEqual([
      '关于 Computer Harness Desktop', '服务', '隐藏 Computer Harness Desktop', '隐藏其他', '全部显示', '退出 Computer Harness Desktop',
    ])
  ...
  it('keeps the English fallback complete', () => {
    const template = macApplicationMenuTemplate('Computer Harness Desktop', 'en')

    expect(template.map(item => item.label)).toEqual([
      'Computer Harness Desktop', 'File', 'Edit', 'View', 'Window',
    ])
    expect(submenu(template[0]!)).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'About Computer Harness Desktop', role: 'about' }),
      expect.objectContaining({ label: 'Quit Computer Harness Desktop', role: 'quit' }),
    ]))
  })
