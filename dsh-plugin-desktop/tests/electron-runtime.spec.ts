  ...
  return {
    ...
    Menu: {
      buildFromTemplate: vi.fn((template: unknown[]) => {
        ...
        if (first?.label === 'Computer Harness Desktop' && Array.isArray(first.submenu)) {
...
describe('Electron desktop runtime', () => {
  beforeEach(() => {
    ...
    updater.filename.mockImplementation((platform: string, version: string) => (
      `Computer-Harness-${version}-${platform === 'darwin' ? 'mac.dmg' : 'windows.exe'}`
    ))
  ...
  it('uses the native macOS frame, Dock icon, and template tray image', async () => {
    ...
    expect(electron.browserWindows[0]?.accessibleTitle).toBe('Computer Harness Desktop')
    ...
    expect(electron.applicationMenuTemplates[0]?.map(item => (item as { label?: string }).label)).toEqual([
      'Computer Harness Desktop', '文件', '编辑', '显示', '窗口',
    ])
  ...
  it('uses the Windows caption, hidden menu bar, removed menu, and fixed blue tray image', async () => {
    ...
    expect(electron.browserWindowOptions[0]).toEqual(expect.objectContaining({
      title: 'Computer Harness Desktop',
  ...
  it('rebuilds ordered effect-scoped tray contributions without replacing native commands', async () => {
    ...
    expect(labels).toEqual([
      'Open Computer Harness Desktop', undefined,
  ...
  it('renders contributed radio submenus in their own profile section', async () => {
    ...
    }>).find(item => item.label === 'Computer Harness Desktop')
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    updater.download.mockResolvedValueOnce('/tmp/Computer-Harness-2.1.0-mac.dmg')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/Downloads/Computer-Harness-2.1.0-mac.dmg',
    })
    ...
    expect(electron.dialog.showSaveDialog).toHaveBeenCalledWith(expect.objectContaining({
      defaultPath: join('/tmp/Downloads', 'Computer-Harness-2.1.0-mac.dmg'),
      filters: [{ name: 'Disk Image', extensions: ['dmg'] }],
    }))
    expect(updater.download).toHaveBeenCalledWith({
      platform: 'darwin',
      version: '2.1.0',
      destinationPath: '/tmp/Downloads/Computer-Harness-2.1.0-mac.dmg',
      request: expect.any(Function),
      signal: controller.signal,
    })
    expect(electron.shell.openPath).toHaveBeenCalledWith('/tmp/Computer-Harness-2.1.0-mac.dmg')
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'darwin',
      version: '2.1.0',
      path: '/tmp/Computer-Harness-2.1.0-mac.dmg',
    })
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
  ...
  it('starts the downloaded Windows installer before requesting orderly exit', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockResolvedValueOnce('C:\\Updates\\Computer-Harness-2.1.0-windows.exe')
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: 'C:\\Updates\\Computer-Harness-2.1.0-windows.exe',
    })
    ...
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'C:\\Updates\\Computer-Harness-2.1.0-windows.exe',
      ['--updated', '--force-run'],
      {
        detached: true,
        stdio: 'ignore',
    ...
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'win32',
      version: '2.1.0',
      path: 'C:\\Updates\\Computer-Harness-2.1.0-windows.exe',
    })
  ...
  it.each([
    ...
    const artifact = {
      ...
      version: '2.0.1',
      path: 'C:\\Updates\\Computer-Harness-2.0.1-windows.exe',
    }
  ...
  it('rejects a macOS handoff when the operating system cannot open the DMG', async () => {
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/Computer-Harness-2.1.0-mac.dmg',
    })
