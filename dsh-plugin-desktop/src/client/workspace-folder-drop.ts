...
const COPY = {
  en: {
    ...
    unavailable: 'Computer Harness Desktop could not read this folder path',
  },
  zh: {
    ready: '松开以添加工作区',
    busy: '正在添加工作区…',
    invalid: '请只拖入一个文件夹',
    unavailable: 'Computer Harness Desktop 无法读取这个文件夹路径',
  },
} as const
...
export async function adoptWorkspaceFolder(
  ...
  if (path.length === 0) throw new Error('Computer Harness Desktop could not read this folder path')
  if (actions.validateDirectory !== undefined && !await actions.validateDirectory(path)) {
    throw new Error('Computer Harness Desktop rejected this workspace location')
  }
