...
const COPY: Record<DesktopLocale, RecoveryCopy> = {
  ...
  zh: {
    title: 'Computer Harness Desktop 恢复',
      ...
      if (action.action === 'home') {
        ...
        await this.runBusy(async () => {
          ...
          this.notice = {
            ...
            body: this.options.locale === 'zh'
              ? '配置选择已保存。请重新启动 Computer Harness Desktop。'
              : 'Profile selection saved. Restart Computer Harness Desktop to apply it.',
          }
          this.restartReady = true
          this.refreshProfiles()
        })
        ...
        await this.runBusy(async () => {
          ...
          this.notice = this.options.locale === 'zh'
            ? { tone: 'success', title: '配置已恢复', body: '已恢复上次成功启动的配置及其快照。请重新启动 Computer Harness Desktop。' }
            : { tone: 'success', title: 'Profile restored', body: 'The last successful Profile and configuration were restored. Restart Computer Harness Desktop to continue.' }
          this.restartReady = true
        })
