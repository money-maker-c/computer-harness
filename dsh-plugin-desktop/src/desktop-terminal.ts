/** Isolated command-line environment launched from the Computer Harness Desktop tray. */
...
/** Platforms with a native terminal launch contract owned by Computer Harness Desktop. */
...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`Computer Harness Desktop ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart Computer Harness Desktop after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("Computer Harness Desktop {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart Computer Harness Desktop after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(Computer Harness Desktop !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart Computer Harness Desktop after plugin changes.')}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "Computer Harness Desktop" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
