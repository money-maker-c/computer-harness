/** Headless-safe npm launcher for the Computer Harness Desktop Electron executable. */
...
Launch Computer Harness Desktop with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate Computer Harness Desktop diagnostics')
    }
    return path.join(appData, 'Computer Harness Desktop')
  }
  if (platform === 'darwin') return path.join(homeDirectory, 'Library', 'Application Support', 'Computer Harness Desktop')
  const config = environment.XDG_CONFIG_HOME
  return path.join(config === undefined || config.length === 0 ? path.join(homeDirectory, '.config') : config, 'Computer Harness Desktop')
}
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged Computer Harness Desktop application.\n',
    )
    return 1
  }
