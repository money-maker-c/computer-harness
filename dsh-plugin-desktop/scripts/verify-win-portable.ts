...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `Computer-Harness-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'Computer Harness Desktop.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing Computer Harness Desktop.exe: ${portablePath}`)
  }
  ...
  assertPortableExecutableBuffer(
    ...
    `${portablePath}:Computer Harness Desktop.exe`,
  )
  return portablePath
}
